...
def read_and_reproject_data(
    ...
    # Capture item metadata for downstream STAC output (written after processing completes
    # so we can rewrite assets to point at the generated COGs/PNGs instead of source S3 hrefs)
...
def build_output_stac_item(
    ...
    """Rewrite the source STAC item so its assets point at generated outputs.

    Asset hrefs are relative to the item file (out/stac_items/<id>.json), so the
    output directory is portable and forms a valid STAC Catalog.
    """
    ...
    def add_asset_if_present(
        ...
        if not os.path.exists(os.path.join(ASSETS_OUT, filename)):
            logger.warning(f"Skipping missing STAC asset: {filename}")
            return
        assets[key] = {
    ...
    add_asset_if_present(
        key="methane-enhancement",
        filename=f"{item_id}_methane_enhancement.tif",
        media_type=COG_MEDIA_TYPE,
        title="Methane enhancement (matched-filter response)",
        roles=["data"],
    )
    add_asset_if_present(
        key="averaged-methane-enhancement",
        filename=f"{item_id}_averaged_methane_enhancement.tif",
        media_type=COG_MEDIA_TYPE,
        title="Regionally averaged methane enhancement",
        roles=["data"],
    )
    add_asset_if_present(
        key="time-signal",
        filename=f"{item_id}_time_signal.json",
        media_type="application/json",
        title="Per-pixel time-signal values inside AOI",
        roles=["metadata"],
    )
    if not skip_colorized:
        add_asset_if_present(
            key="methane-enhancement-colorized",
            filename=f"{item_id}_methane_enhancement_colorized.tif",
            media_type=COG_MEDIA_TYPE,
            title="Colorized methane enhancement heatmap",
            roles=["visual", "overview"],
        )
    if not skip_viz:
        add_asset_if_present(
            key="thumbnail",
            filename=f"{item_id}_methane_enhancement.png",
            media_type="image/png",
            title="Methane enhancement preview",
            roles=["thumbnail"],
        )
...
def write_stac_output(item_dict: dict[str, Any], item_id: str) -> None:
    """Write the per-item STAC JSON and a wrapping STAC Catalog to OUT_DIR."""
    item_path = os.path.join(STAC_ITEMS_OUT, f"{item_id}.json")
    with open(item_path, "w") as f:
        json.dump(item_dict, f, indent=2)
    logger.info(f"Saved STAC item: {item_path}")

    write_stac_catalog([item_id])


def write_stac_catalog(item_ids: list[str]) -> None:
    """Write a STAC Catalog linking all successfully processed item IDs."""
    catalog = {
        "type": "Catalog",
        "id": "methane-detection-output",
        "stac_version": "1.0.0",
        "description": "Methane enhancement products generated by the matched-filter detector.",
        "links": [
    ...
    logger.info(f"Saved STAC Catalog: {catalog_path}")
...
def main(
    ...
    try:
        ...
        output_item = build_output_stac_item(
        ...
        write_stac_output(output_item, l1c_item_id)
